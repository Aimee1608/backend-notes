...
> 第一篇把 token 怎么工作讲清楚了(机制)。但真到做项目,问题往往不是"JWT 怎么签",而是"我这个场景到底该用什么方案、自己搭还是用现成的、有哪些坑别踩"。
> 这篇就是一份**按场景查的选型 + 避坑指南**。全文基于公开行业方案与标准。(缩写较多,首次出现都用括号点一下,文末也有「名词速查」。)
...
- **PC 网站扫码登录**:要 **微信开放平台 + 企业资质 + 300 元/年审核**,**个人开发者做不了**。
- **公众号 H5 网页登录**:要 **认证服务号**(企业认证,300 元/年)+ 配**网页授权域名**(域名得 ICP 备案);授权分两种 `scope`——`snsapi_base`(静默,只拿 openid)、`snsapi_userinfo`(弹窗,能拿头像昵称)。
...
> **选型提示**:个人项目想要"微信登录",常卡在资质门槛——这时要么先用 GitHub / Google 这种零门槛的,要么用 IDaaS(Auth0 / Authing 把微信接入封装好了,省去你自己搞资质和对接,见 Part 2)。
...
**① 邮箱登录**
- 实现:邮箱 + 密码(密码必须 **bcrypt / argon2 哈希**),或验证码 / **magic link**(发一个登录链接,点击即登录,无密码、体验好)。
- 门槛 / 坑:
  - 发信要接 **SMTP 服务**(SendGrid、阿里云邮件推送、Mailgun)——**别自建邮件服务器**,送达率惨。
  - 邮件易**进垃圾箱**,要给域名配 **SPF / DKIM / DMARC**(邮件域名认证)提高送达率、防伪造。
  - 注册时发链接 / 验证码**验证邮箱所有权**,别让人拿别人邮箱乱注册。
...
> 系列导航:上一篇 [《JWT、OAuth、Token 刷新 —— 鉴权入门》](./01-jwt-oauth-basics.md) 讲机制;这篇讲场景选型与避坑。下一篇会挑一个场景**深入展开**(如自建登录体系,或 SSO 落地)。
