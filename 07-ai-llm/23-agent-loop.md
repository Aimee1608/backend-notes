...
这个骨架三年前叫 ReAct（Reasoning + Acting：想一步、做一步）。现在的模型把"想"内化到了思考模式里，循环本身简化了——不需要在 prompt 里要求模型先写"Thought:"再写"Action:"，直接让它返回工具调用就行。骨架没变，变的是模型在每一圈里的可靠性。

主流 SDK 现在都提供了这个循环的封装：你只写工具函数，SDK 负责调模型、执行、回传、判停。自己写循环的价值在于**每一圈都有钩子**——审批、拦截、记账、改写结果——生产环境通常需要这些。
...
好处是每个循环的上下文都短而专注，坏处是协调本身也是一层复杂度。规律：**单个循环超过二十步、或者上下文超过窗口四分之一还在涨，就该考虑拆了**。第 13 篇的"隔离"说的就是这个。
