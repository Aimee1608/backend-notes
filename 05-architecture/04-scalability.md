...
> 业务一涨，单机迟早到顶——CPU 跑满、内存吃光、连接数撑爆。扩容只有两条路：把机器升配（总有物理上限），或者多搬几台一起扛（理论上可以一直加）。这篇拆开讲**"加机器就能线性扛更多"**是怎么做到的：前提条件、请求怎么分发、最难啃的数据层怎么扩。

---

先上一张全景——这一篇要搭的,就是下面这套"加机器就能扛更多"的架构:用户请求先过**负载均衡**,分给一堆**无状态应用实例**,后面接**共享缓存**和**读写分离的数据层**。后面几节就顺着这张图逐层拆开。

![可扩展性 · 水平扩展架构全景](img/04-scalability.png)
